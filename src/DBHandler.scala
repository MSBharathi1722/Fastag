...
class DBHandler{
	...
	def updateBalance(id:Int , amount:Int,operator:String):String = {
		getConnection()
	    var result:String = "false"
	    try {
	      stmt = con.prepareStatement("update balance_detail set balance = balance"+operator+"? where user_id=?")
	...
	def getAmount(user_id : Int):Int = {
		...
		try{
			...
			if (stmt.execute()) {
				rs = stmt.getResultSet
	        	while (rs.next()) {
	        		var vehicle_type: String = rs.getString(1)
				    var amt:Int = 0
				    vehicle_type match {
				    	case "type1" => amt = 85
				    	case "type2" => amt = 135
